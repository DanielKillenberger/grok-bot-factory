...
The factory does not call `/land`. The coordinator merges. Do not invoke land. Do not invent `/pilot`. Do not launch `/flow-next:impl-review`. Later phases stay on the spec branch or that spec's open PR head. Do not follow a generated cursor branch if one appeared.

## Lease and cap

In flight means a live Cloud Agent run for that spec, a live per-spec check, or this coordinator turn. Lease key is repo `full_name` plus spec id. Persist the lease and a deterministic client agent id on this Bot computer before the launch POST. Then POST, write run id, arm that spec's named 30-minute check, write check-routine id. On crash, reconcile from the lease: a lease with no run id retries the POST; a run without a check arms the check. A complete lease (run id and an armed check) is already in flight. Do not launch a second agent for that key until that job finishes.

A JSON ledger entry alone is not a check. Creating the check must arm a real Grok Bot 30-minute scheduled routine that wakes this coordinator for that spec, or a routine whose first action re-POSTs the shipped factory webhook (`bun factory/check-fire.ts --repo owner/name --spec <id>`, same `GROK_BOT_WEBHOOK_URL` / `GROK_BOT_SENDER_KEY` Bearer path as factory-forward). Persist that routine handle. Deleting or cancelling the check must disarm that routine. Do not invent a public Grok Bot REST client. If the routine cannot be created, fail closed and ping.

Reserve a factory slot under one atomic file lock on this Bot computer (same `wx` lock shape as `factory/lib/lock.ts`). Cap is 10. An 11th in-flight spec never starts. Extra ready specs wait. No ping for a full cap. The per-Bot routine cap of 50 remains a backstop ping if a check cannot be armed.
...
When a Cloud Agent finishes or errors, that wakes this coordinator. The revival payload is finished plus a transcript dump, not a verdict. The wake names the run that finished. If that run id is not the current lease run, ignore it and do not cancel the live check. Cancel the 30-minute check that was watching that run first. Then GET the run — run status (`FINISHED` / `ERROR` / `RUNNING`) is truth, not the done-wake hint. If that GET fails or reports RUNNING, recreate the per-spec 30-minute check and judge; do not leave the spec without a hang detector. If the check cannot be recreated, ping NEEDS_HUMAN. Do not hold without a live clock. Hang remains the 30-minute clock, not the done-wake. Read the result from the agent or from git using the persisted ids. A FINISHED run with no readable artifact is unknown. Do not treat it as phase done.

The 30-minute check is the guaranteed detector if the done-wake never fires. A check fire that finds a finished or errored run is the same as a done-wake: cancel that build-run check, then act. Do not use the cloud done-wake to cover hang.

Each spec has at most one 30-minute routine. Cancel means disarm the hang detector for a finished Cloud Agent. After make-pr, create or retarget that same per-spec routine as a PR watch. Do not keep a cancelled build-run check firing. Do not orphan-delete an open-PR check.
...
3. no agent and no open PR, lease live, spec still open → missed wake: keep the lease and continue from git (flow-next on disk); launch the next child. Do not clear the lease. Do not stop. Clearing and stopping is idle-until-push.
4. agent still running → hang. Escalate-ping (`NEEDS_HUMAN`). Do not hold. Hang is the 30-minute clock firing while the child is alive, not N looks. No look-count auto-ping.

Orphan-delete only when the spec is merged/cleared, or a PR-watch finds no PR.
...
Judgment uses the result plus this spec's history. Rounds and look counts are inputs, not caps. Eight rounds can be correct. Three can be enough to ask. A 30-minute check fire while the cloud agent is still running is hang: escalate-ping. No round number or look count auto-pings.

After make-pr, watch CI and reviews. Merge when the work is done. Dispatch a fix agent when CI or review needs a fix. The factory does not call `/land`. The coordinator merges. You merge. Do not invoke land.
