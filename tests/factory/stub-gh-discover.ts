...
if (/\bsecret\b/.test(args) || args.includes("/actions/secrets")) {
  appendFileSync(`${log}.secrets`, `${args}\n`);
  console.error("gh: secret mutate is forbidden on the discover path");
  process.exit(1);
}
if (args.includes("--method PUT") || args.includes("--method PATCH") || args.includes("--method POST")) {
  appendFileSync(`${log}.mutate`, `${args}\n`);
  console.error("gh: mutate REST is forbidden on the discover path");
  process.exit(1);
}
