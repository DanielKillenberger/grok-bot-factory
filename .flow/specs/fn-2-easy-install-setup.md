...
4. Confirm handoff (R3): the skill (conversation coordinator) never invokes Action install or routine create until an explicit confirmation reply names the subset. Unconfirmed candidates never reach mutate. The mutate program is a skill→program boundary: it refuses discover/candidate-shaped input (`candidates` JSON, `--candidates`); it cannot cryptographically prove a list was confirmed.
