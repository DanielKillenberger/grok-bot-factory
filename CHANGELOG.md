...
- `hostRun` also polls `~/.grok/sessions/<encodeURIComponent(realpath(tree))>/**/{chat_history,updates}.jsonl` (that tree only) for the same verdict line in raw file text, because grok TUI does not paint `PILOT_VERDICT` onto the PTY. First match SIGTERMs the process group and returns the line, same as a typescript hit.
- `hostRun` (and tick) only treat `PILOT_VERDICT=` / `LAND_VERDICT=` as a hit when the value is `ADVANCED|ASKED|NO_WORK|DEFERRED_TO_LAND|BLOCKED|NEEDS_HUMAN`. The `/loop` prompt template `PILOT_VERDICT=<ADVANCED|…>` is ignored.
